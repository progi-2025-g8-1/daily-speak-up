...
<template>
    ...
    <div class="flex flex-col
        ...
        <div v-if="isLoading" class="w-full max-w-7xl
                                  h-full space-y-6">
        ...
        <div v-else-if="videos.length === 0" class="flex flex-col items-center justify-center h-full py-20">
            <i class="pi pi-check-circle text-6xl mb-4" style="color: var(--color-success)"></i>
            <h3 class="text-2xl font-semibold mb-2 no-reports-title">{{ t('admin_dashboard.reported_videos.no_reports_title') }}</h3>
            <p class="no-reports-message">{{ t('admin_dashboard.reported_videos.no_reports_message') }}</p>
        </div>
